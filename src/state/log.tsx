...
import type { FruitLog, FruitLogEntry } from '../core'
...
export function LogProvider({ children }: { children: ReactNode }) {
  ...
  const [log, setLog] = useState<FruitLog>(() => loadLocal(scope))
  const [pending, setPending] = useState(0)
  const [syncing, setSyncing] = useState(false)
  const logRef = useRef(log)
  logRef.current = log

  // スコープ（ユーザー）切替時にローカルを読み直し。
  useEffect(() => {
    const local = loadLocal(scope)
    setLog(local)
  ...
  // ── Supabase からの初回取り込み＋オフラインキュー flush ──
  ...
  }, [log, user, pending, syncing, save, remove])
